        ...
        <tbody className="divide-y divide-gray-100">
          {orders.map((o) => {
            ...
            const statusLabel =
              ...
              String(o.status || '—')
            const processedByName = resolveOrderProcessedBy(o)
            const statusBadgeText = buildAdminStatusBadgeText(statusLabel, processedByName)
            const statusBadgeWeight = processedByName ? 'font-semibold' : 'font-bold'
            return (
              <tr key={o._id} className="hover:bg-gray-50/60">
                ...
                <td className="max-w-[14rem] px-4 py-3">
                  <span
                    className={`inline-flex max-w-full truncate rounded-full px-2.5 py-0.5 text-xs ${statusBadgeWeight} ${badgeClass}`}
